...
"""
Test FlashAttention plugin system integration with DotProductAttention.

This test verifies that DotProductAttention can use the plugin system's
FlashAttention when available, and falls back to native implementation otherwise.
"""
...
def test_tex_flash_attention_availability():
    """Test 1: Check if tex module has flash_attention method"""
    print_section("Test 1: Check tex module for flash_attention")

    try:
        import transformer_engine_torch as tex

        has_flash_attention = hasattr(tex, 'flash_attention')
        is_callable = callable(getattr(tex, 'flash_attention', None))
        ...
        if has_flash_attention and is_callable:
            ...
            print_info("(This is expected if using native tex module)")
            return False
...
def test_dotproductattention_code_changes():
    """Test 2: Verify DotProductAttention has correct modifications"""
    ...
    try:
        ...
        for i, line in enumerate(lines[58:75], start=58):
            ...
            if 'FlashAttention = getattr(tex, \'flash_attention\'' in line:
                found_getattr = True
                getattr_line = i + 1
...
def test_plugin_system_integration():
    """Test 3: Test actual plugin system integration"""
    print_section("Test 3: Test plugin system integration")

    try:
        # Replace tex module with plugin system
        import transformer_engine.plugins.transformer_engine_fl_torch as tex_plugins
        sys.modules['transformer_engine_torch'] = tex_plugins

        print_info("Replaced transformer_engine_torch with plugin system")

        # Check if flash_attention is available
        has_flash_attention = hasattr(tex_plugins, 'flash_attention')
        is_callable = callable(getattr(tex_plugins, 'flash_attention', None))
        ...
        # Try to import DotProductAttention
        ...
        # Check the FlashAttention backend
...
def test_backend_switching():
    """Test 4: Test backend switching with environment variable"""
    print_section("Test 4: Test backend switching")

    try:
        backends_to_test = ["flaggems", "torch"]

        for backend in backends_to_test:
            print_info(f"\nTesting backend: {backend}")

            # Set backend
            os.environ["TE_BACKEND"] = backend

            # Reload registry to pick up new backend
            import importlib
            import transformer_engine.plugins.tex_interface.registry as registry_module
            importlib.reload(registry_module)

            # Reload tex_plugins
            import transformer_engine.plugins.transformer_engine_fl_torch as tex_plugins
            importlib.reload(tex_plugins)
            sys.modules['transformer_engine_torch'] = tex_plugins

            # Create DotProductAttention instance
            from transformer_engine.pytorch.attention.dot_product_attention import DotProductAttention
...
def main():
    """Run all FlashAttention integration tests"""
    print_header("FlashAttention Plugin System Integration Tests")

    results = []

    # Test 1: Check tex module
    results.append(("tex.flash_attention availability", test_tex_flash_attention_availability()))
    ...
    if passed == total:
        ...
        print_info("  1. Set backend: export TE_BACKEND=flaggems")
        print_info("  2. Replace tex module in your code:")
        print_info("     import transformer_engine.plugins.transformer_engine_fl_torch as tex")
        print_info("     sys.modules['transformer_engine_torch'] = tex")
