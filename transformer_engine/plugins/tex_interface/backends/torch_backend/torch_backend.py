...
"""
PyTorch Native Backend for TEX Interface

This backend uses PyTorch native operations for all computations.
It serves as a reference implementation and fallback option.

Priority: 100 (lower than FlagGems but can be used independently)
"""
...
def debug_print(func_name: str, *args, **kwargs):
    ...
    if DEBUG:
        debug_print_once(func_name, "TorchBackend", *args, **kwargs)


@register_backend
class TorchBackend(TEXBackendBase):
    """
    PyTorch Native Backend.

    This backend provides:
    1. Pure PyTorch implementations of all operations
    2. Reference implementation for testing
    3. Fallback option when other backends are not available
    ...
    def get_flash_attention_class(self):
        """Return the FlashAttention class for Torch backend."""
    ...
    def multi_tensor_unscale_l2norm(self, chunk_size: int, noop_flag: torch.Tensor, tensor_lists: List[List[torch.Tensor]], scale: torch.Tensor, per_tensor: bool = False) -> Union[torch.Tensor, List[torch.Tensor]]:
        # First unscale tensors, then compute norm
        if noop_flag.item() != 0:
            if per_tensor:
        ...
        # Compute norm on unscaled tensors
    ...
    def multi_tensor_adam_param_remainder(self, *args, **kwargs) -> None:
        # This is a specialized variant, not implemented yet
    ...
    def multi_tensor_adam_fp8(self, *args, **kwargs) -> None:
        # FP8-specific Adam, not implemented yet
    ...
    def multi_tensor_adam_capturable(self, *args, **kwargs) -> None:
        # CUDA graph capturable Adam, not implemented yet
    ...
    def multi_tensor_adam_capturable_master(self, *args, **kwargs) -> None:
        # CUDA graph capturable Adam with master weights, not implemented yet
    ...
    def multi_tensor_compute_scale_and_scale_inv(self, *args, **kwargs) -> None:
        # FP8-specific operation, not implemented yet
