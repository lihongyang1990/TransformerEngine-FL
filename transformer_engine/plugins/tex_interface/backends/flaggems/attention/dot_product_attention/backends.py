...
# Import FlashAttention base class (use absolute import to avoid issues with dynamic module loading)
from transformer_engine.plugins.tex_interface.flash_attention_base import FlashAttentionBase

# Use absolute import to avoid issues with dynamic module loading
from transformer_engine.plugins.tex_interface.backends.flaggems.import_utils import have_flag_gems
from transformer_engine.plugins.tex_interface.logger import print_once
...
class AttnFuncFL(torch.autograd.Function):
    ...
    def forward(
        ...
        # Debug logging for FlagGems FlashAttention - print only once
        if _DEBUG:
            print_once(f"[flaggems] Calling flash_attention_forward\n"
        ...
        if _DEBUG:
            print_once(f"[flaggems] flash_attention_forward completed successfully\n"
    ...
    def backward(ctx, d_out, *_args):
        ...
        with torch.cuda.nvtx.range("AttnFuncFL.backward"):
            ...
            # Debug logging for FlagGems FlashAttention backward - print only once
            if _DEBUG:
                print_once(f"[flaggems] Calling flash_attention_backward\n"
            ...
            if _DEBUG:
                print_once(f"[flaggems] flash_attention_backward completed successfully\n"
...
class FlashAttentionFL(FlashAttentionBase):
    """
    FlagGems FlashAttention implementation.

    This class implements FlashAttention using the flag_gems library,
    providing an optimized attention implementation for various hardware.
    """
